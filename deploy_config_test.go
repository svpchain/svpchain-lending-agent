...
// ~/.config/svpchain-lending-agent/config.sh, and a developer who has one would
...
func TestDeployScriptConfigParses(t *testing.T) {
	...
	for name, args := range cases {
		t.Run(name, func(t *testing.T) {
			...
			// rewrite this to /etc/svpchain-lending-agent/run/secrets/… and
...
// ~/.config/svpchain-lending-agent/config.sh so a routine install takes no flags.
...
func TestDeployScriptReadsConfigFile(t *testing.T) {
	...
	t.Run("scalars come from the file", func(t *testing.T) {
		...
		if cfg.PublicURL != "https://agents.example.org/lending" {
