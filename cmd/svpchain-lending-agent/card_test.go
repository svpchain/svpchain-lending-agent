...
func TestCardMatchesGolden(t *testing.T) {
	...
	wire.LendingProfile.Register(reg, &tools.Handlers{}, agentchain.New(nil, nil, nil, nil, nil, nil, nil), nil)

	got, err := json.Marshal(a2aserver.BuildAgentCardFor(identity, "https://agents.example.test/lending", reg))
	if err != nil {
		t.Fatal(err)
	}
...
func TestCardMatchesRegistry(t *testing.T) {
	reg := toolbridge.NewEmpty()
	wire.LendingProfile.Register(reg, &tools.Handlers{}, agentchain.New(nil, nil, nil, nil, nil, nil, nil), nil)
	card := a2aserver.BuildAgentCardFor(identity, "https://agents.example.test/lending", reg)
