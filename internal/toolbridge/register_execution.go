...
var executionLendoraTools = []string{
	...
	"execute_lendora_borrow",
	"execute_lendora_repay",
}

// RegisterExecutionLendora adds the delegated Lendora money-market writes. A
// nil service, or one without the Lendora comptroller configured, registers
// the same tool names as refusals so a caller learns the requirement.
func (r *Registry) RegisterExecutionLendora(s *delegated.Service) {
	if s == nil {
		for _, tool := range executionLendoraTools {
			r.addRefusing(SkillExecutionLendora, tool, keylessReason)
		}
		return
	}
	...
	r.add(SkillExecutionLendora, "execute_lendora_borrow", adaptStrictNative(s.ExecuteLendoraBorrow))
	r.add(SkillExecutionLendora, "execute_lendora_repay", adaptStrictNative(s.ExecuteLendoraRepay))
}
