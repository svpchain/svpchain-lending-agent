...
const (
	...
	SkillExecutionLendora = "svpchain-execution-lendora"
)
...
// RegisterEVMBroadcast adds the EVM landing rail — broadcast a raw tx and track
// its status. This is the whole of the EVM surface this binary serves: Lendora
// builds return EVM payloads that land here. The swap, bridge and ERC building
// tools that used to share this skill went with the EVM DeFi family, which is
// another binary's surface; cmd/svpchain-lending-agent/card.go overrides the
// skill description so the card says so.
...
// RegisterLendora adds the Lendora money market.
func (r *Registry) RegisterLendora(h *tools.Handlers) {
