...
import (
	...
	"github.com/svpchain/svpchain-lending-agent/internal/mcp/faucet"
	"github.com/svpchain/svpchain-lending-agent/internal/mcp/indexer"
	"github.com/svpchain/svpchain-lending-agent/internal/mcp/lendora"
	...
	"github.com/svpchain/svpchain-lending-agent/internal/agentrest"
...
type App struct {
	...
	Lendora  *lendora.Cache       // must Run(ctx); nil unless lendora is configured
...
// Run starts the Lendora cache refresher and blocks until ctx is cancelled.
//
// The CLOB markets cache went with the perps surface — this agent prices
// against Lendora's own market data, not perp metadata. The Lendora cache is
// nil only when the comptroller is unconfigured, which main.go already refuses
// at boot, so in a real deployment there is always exactly one refresher.
...
func BuildProfile(ctx context.Context, cfg *config.Config, p Profile) (*App, error) {
	...
	// Lendora is this binary's whole reason to exist, so main.go calls
	// cfg.RequireLendora() before wiring: a missing EVM endpoint or comptroller
	// is a boot failure here, not a call-time refusal. The guard below therefore
	// never fails in a real deployment; it stays so wire is still callable from
	// tests with a bare config.
	//
	// Only the landing rail and the money market are built. The swap, oracle and
	// bridge bindings went with the EVM DeFi surface this agent does not serve —
	// constructing them would dial endpoints and read routes.json for families
	// it would only refuse.
