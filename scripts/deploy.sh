...
#
# This agent serves the Lendora money-market slice of the SVP-Chain A2A surface
# on :8084, advertised at <public-url>/lending. It is deployed independently: its
...
# the image tar to ~/svpchain-lending-agent → docker load → docker compose up -d
...
#   ~/.config/svpchain-lending-agent/config.sh
#
...
#
#   SVPCHAIN_LENDING_AGENT_OPERATOR_KEY
...
#                                    SVPCHAIN_LENDING_AGENT_OPERATOR_KEY="$(op read …)"
...
#   --evm-rpc <url>                The chain's EVM JSON-RPC. Required for the
#                                  Lendora family; empty → it refuses.
#                                  SVPCHAIN_EVM_RPC
...
readonly AGENT_SEGMENT="lending"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --evm-lendora-comptroller) evm_lendora_comptroller="$2"; mark_flag SVPCHAIN_EVM_LENDORA_COMPTROLLER; shift 2 ;;
...
EOF
  # The Lendora family dials the chain's EVM JSON-RPC; without it the
  # comptroller binding is never built and the Lendora tools refuse.
  ...
  # The Lendora money market on the DEX chain's EVM side. Core requires
  # evm_rpc_url alongside it, so the two are set or unset together.
...
validate_hex_key() {
  [[ "$1" =~ ^(0x)?[0-9a-fA-F]{64}$ ]] \
    || fail "SVPCHAIN_LENDING_AGENT_OPERATOR_KEY does not look like a 32-byte hex key (got ${#1} characters)"
}

# resolve_operator_key — normalise and validate the key material supplied in
# SVPCHAIN_LENDING_AGENT_OPERATOR_KEY. Without one the agent runs keyless: it
...
if [[ "$mode" == "print-env" ]]; then
  ...
  for _i in "${!env_names[@]}"; do
    ...
    if [[ "$name" == "SVPCHAIN_LENDING_AGENT_OPERATOR_KEY" ]]; then
...
if [[ "$skip_build" == "1" ]]; then
  ...
  # Vendored build (see cmd/svpchain-lending-agent/Dockerfile): the go.mod
...
if [[ "$dry_run" == "1" ]]; then
  ...
  if [[ -z "$healthy" ]]; then
    ...
    info "from inside the container. This agent also needs evm_rpc_url and the"
    info "Lendora comptroller address to serve anything beyond refusals."
