#!/usr/bin/env bash
#
# scripts/deploy.sh — install the svpchain lending agent onto a remote SSH host
# as a docker container.
#
# This agent serves the lending slice of the SVP-Chain A2A surface on
# :8082, advertised at <public-url>/lending. It is deployed independently: its
...
# ~/svpchain-lending-agent → docker load → docker compose up -d → smoke-test
...
#   --public-url <url>             Base URL; this agent advertises <base>/lending.
...
#   --install-dir <path>           Default ~/svpchain-lending-agent on remote.
#   --print-config / --print-compose / --print-routes / --dry-run / --uninstall
#
# Examples:
#   ./scripts/deploy.sh --host www@svpdev1.example.com
#   ./scripts/deploy.sh --host www@svpdev1.example.com \
#     --operator-key-file ./lending.key --public-url https://agents.svpchain.org
...
operator_metadata=""
print_agent="svpchain-lending-agent"  # which agent --print-config renders
...
install_dir="~/svpchain-lending-agent"
image_tag=""
...
# at call time.
render_agent_toml() {
  local name="${1:-svpchain-lending-agent}"
  local port="${2:-$(agent_port "$name")}"
  # Advertised URL: the base plus this agent's path segment (/research,
  # /lending, /evm, /lending) — a reverse proxy routes each path to that agent.
...
render_compose_yaml() {
  ...
  for name in "${AGENTS[@]}"; do
    ...
    # ENTRYPOINT ["/bin/svpchain-lending-agent"], and compose `command:`
    ...
    # `svpchain-lending-agent svpchain-lending-agent -config …` and die on flag
    # parsing.
...
if [[ "$mode" == "print-config" ]]; then
  # Preview one agent's config (default svpchain-lending-agent; --print-agent
...
if [[ "$mode" == "print-compose" ]]; then
  ...
  resolve_operator_keys
  image_ref="ghcr.io/svpchain/svpchain-lending-agent:${image_tag:-<tag>}"
  render_compose_yaml
  exit 0
fi
...
image_ref="ghcr.io/svpchain/svpchain-lending-agent:${image_tag}"
image_tar="${REPO_DIR}/build/svpchain-lending-agent.image.tar"
...
if [[ "$skip_build" == "1" ]]; then
  ...
  build_cmd+=" -t ghcr.io/svpchain/svpchain-lending-agent:latest"
  build_cmd+=" -f cmd/svpchain-lending-agent/Dockerfile ."
...
compose_tmp="$(mktemp -t svpchain-lending-agent.compose.XXXXXX)"
routes_tmp=""
...
if [[ -n "$bridge_routes_basename" ]]; then
  if [[ -n "$bridge_routes_src_abs" ]]; then
    ...
    routes_tmp="$(mktemp -t svpchain-lending-agent.routes.XXXXXX)"
...
run_or_print "rsync -avz '$image_tar' '$host:$install_dir/svpchain-lending-agent.image.tar'"

# Phase 4: load (On remote)
step "On remote: docker load (skipped if image already loaded)"
load_if_missing "$image_ref" "$install_dir/svpchain-lending-agent.image.tar" "$expected_id"
remote_exec "docker tag $image_ref ghcr.io/svpchain/svpchain-lending-agent:latest"
